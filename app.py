...
def render_dashboard():
    ...
    if total_used < 100000 and len(df_open[df_open['TradeType'].isin(['STOCK', 'LEAP'])]) > 10:
        ...
        if len(missing_prices) > 0:
            ...
            st.error(f"⚠️ **Warning:** {len(missing_prices)} positions with {total_shares_missing:,.0f} shares have missing prices. Prices will be fetched from Yahoo Finance, or update prices in Excel to see accurate capital deployment.")
...
def render_margin_config():
    ...
    if len(zero_capital) > 0 and not live_prices:
        st.warning(f"⚠️ {len(zero_capital)} ticker/type combinations show $0 capital due to missing prices. Prices will be fetched from Yahoo Finance, or update prices in Excel.")
...
def main():
    ...
    if errors and df_trades is None:
        ...
        st.error(f"❌ {errors[0]}")
        if not data_file.exists():
            st.info(f"💡 **File not found**: `{data_file.name}`\n\n"
